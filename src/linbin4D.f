...
c Last changed: 31 AUG 2005
      ...
      subroutine lbfoud(X,n,a1,a2,a3,a4,b1,b2,b3,b4,M1,M2,M3,M4,
     + w,gcounts)  
      ...
      integer ind15,ind16
      double precision X(*),a1,a2,a3,a4,b1,b2,b3,b4,gcounts(*),w(*)
...
c     Find the integer part of "lxi1","lxi2", "lxi3" and "lxi4"
       ...
       gcounts(ind1) = gcounts(ind1) 
     +      + w(i)*((1-rem1)*(1-rem2)*(1-rem3)*(1-rem4))
       gcounts(ind2) = gcounts(ind2)
     +      + rem1*(1-rem2)*(1-rem3)*(1-rem4)
       gcounts(ind3) = gcounts(ind3)
     +      + (1-rem1)*rem2*(1-rem3)*(1-rem4)
       gcounts(ind4) = gcounts(ind4)
     +      + rem1*rem2*(1-rem3)*(1-rem4)
       gcounts(ind5) = gcounts(ind5)
     +      + (1-rem1)*(1-rem2)*rem3*(1-rem4)
       gcounts(ind6) = gcounts(ind6)
     +      + rem1*(1-rem2)*rem3*(1-rem4)
       gcounts(ind7) = gcounts(ind7)
     +      + (1-rem1)*rem2*rem3*(1-rem4)
       gcounts(ind8) = gcounts(ind8)
     +      + rem1*rem2*rem3*(1-rem4)

       gcounts(ind9)  = gcounts(ind9) 
     +      +(1-rem1)*(1-rem2)*(1-rem3)*rem4
       gcounts(ind10) = gcounts(ind10)
     +      +rem1*(1-rem2)*(1-rem3)*rem4
       gcounts(ind11) = gcounts(ind11)
     +      +(1-rem1)*rem2*(1-rem3)*rem4
       gcounts(ind12) = gcounts(ind12)
     +      +rem1*rem2*(1-rem3)*rem4
       gcounts(ind13) = gcounts(ind13)
     +      +(1-rem1)*(1-rem2)*rem3*rem4
       gcounts(ind14) = gcounts(ind14)
     +      +rem1*(1-rem2)*rem3*rem4
       gcounts(ind15) = gcounts(ind15)
     +      +(1-rem1)*rem2*rem3*rem4
       gcounts(ind16) = gcounts(ind16)
     +      +rem1*rem2*rem3*rem4
                              endif
                           endif 
                        endif
