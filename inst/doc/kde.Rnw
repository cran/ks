...
\begin{center}
<<fig=TRUE,echo=FALSE,print=FALSE>>=
plotmixt(mus, Sigmas, props, display="image", col=rev(heat.colors(100)), cex.axis=1.6, cex.lab=1.6)
@ 
<<fig=TRUE,echo=FALSE,print=FALSE>>=
fhat.diag <- kde(x, H=H.pidiag, binned=TRUE, bgridsize=rep(51,2))
plot(fhat.diag, display="image", col=rev(heat.colors(100)), cex.axis=1.6, cex.lab=1.6)
@ 
\end{center}
